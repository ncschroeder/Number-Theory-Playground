...
class AncientMultiplicationTests {
    ...
    1. input1 is a power of 2 so its binary string consists of a 1 followed by either nothing or
       some 0's. There, of course, is only 1 power of 2 that sums to input1. table2Rows will only
       have a single row.
    2. input1 is 1 less than a power of 2 so its binary string consists only of 1's and the powers
       of 2 that sum to input1 are all the powers of 2 less than it. table1Rows and table2Rows will
       have the same rows.
    3. None of the above. The binary string of input1 will consist of at least two 1's and at least
       one 0. The amount of powers of 2 that sum to input1 will be > 1 and < the amount of powers
       of 2 < input1. table2Rows will have more than 1 row but won't have as many rows as table1Rows.
